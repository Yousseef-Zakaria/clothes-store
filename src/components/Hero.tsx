...
const HeroSection = () => {
  return (
    <div className="relative w-full min-h-screen overflow-hidden bg-[#F8F9FA] text-zinc-900 selection:bg-lime-400 selection:text-black">
      
      {/* --- BACKGROUND LAYER --- */}
      <div className="absolute inset-0 z-0 bg-[#F8F9FA]">
        
        {/* Mobile Only: Image with multiply blend mode for clean integration */}
        <img 
          src="heroSection.jpg" 
          alt="Mobile Background Overlay"
          className="lg:hidden absolute inset-0 w-full h-full object-cover grayscale opacity-[0.08] mix-blend-multiply" 
        />

        {/* Universal: Grainy overlay for subtle texture (reduced opacity for light mode) */}
        <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.04] z-10 pointer-events-none"></div>
        
        {/* Desktop Only: Airy, bright glowing orbs */}
        <div className="hidden lg:block absolute top-1/4 right-1/4 w-[600px] h-[600px] bg-lime-300/20 rounded-full blur-[120px] pointer-events-none"></div>
        <div className="hidden lg:block absolute bottom-1/4 left-1/4 w-[500px] h-[500px] bg-cyan-200/20 rounded-full blur-[100px] pointer-events-none"></div>
      </div>

      {/* --- NAVIGATION BAR --- */}
      <nav className="fixed top-0 w-full z-50 transition-all duration-300 backdrop-blur-xl bg-white/70 border-b border-zinc-200/50 shadow-sm">
        <div className="flex justify-between items-center px-6 md:px-8 py-4 max-w-7xl mx-auto">
          {/* Mobile Menu Icon */}
          <button className="md:hidden text-zinc-900 hover:text-lime-500 transition-colors">
            <Menu size={28} />
          </button>
          ...
          <img
            src="logo.jpg"
            alt="logo"
            className="h-12 w-12 md:h-16 md:w-16 object-cover rounded-full shadow-md border border-zinc-200" 
          />

          <ul className="hidden md:flex space-x-10 font-medium text-sm tracking-widest uppercase text-zinc-700" dir="rtl">
            <li className="hover:text-zinc-900 transition-colors cursor-pointer relative group">
              الكوليكشنز
              <span className="absolute -bottom-2 right-0 w-0 h-0.5 bg-lime-400 transition-all group-hover:w-full"></span>
            </li>
            <li className="hover:text-zinc-900 transition-colors cursor-pointer relative group">
          ...
          {/* Cart Button - Inverted to Dark for High Contrast */}
          <button className="flex cursor-pointer items-center gap-2 bg-zinc-900 text-white px-5 py-2 md:py-2.5 rounded-full font-bold text-sm hover:bg-lime-400 hover:text-black hover:shadow-[0_0_20px_rgba(163,230,53,0.5)] transition-all duration-300">
            <ShoppingBag size={18} />
            <span className="hidden md:inline">سلة المشتريات</span>
            <span className="bg-white/20 px-2 py-0.5 rounded-full text-xs">2</span>
          </button>
        </div>
      </nav>
      ...
      <div className="relative z-20 flex flex-col lg:flex-row items-center justify-between min-h-screen px-6 md:px-12 max-w-7xl mx-auto pt-24 pb-24 gap-12" dir="rtl">
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center items-center lg:items-start pt-10 lg:pt-0 z-20 text-center lg:text-right">
          ...
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2, duration: 0.8 }}
            className="flex items-center gap-3 mb-6 bg-white border border-zinc-200 px-4 py-2 rounded-full shadow-sm"
          >
            <span className="w-2.5 h-2.5 rounded-full bg-lime-500 animate-pulse"></span>
            <p className="text-zinc-600 font-mono text-xs md:text-sm tracking-widest uppercase font-semibold">
              كوليكشن Box fit جديد قادم 05
            </p>
          </motion.div>
          ...
          <motion.h1 
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.4, duration: 0.8, ease: "easeOut" }}
            className="font-arabic-heavy italic text-6xl md:text-7xl lg:text-8xl font-black leading-[1.1] text-zinc-900 drop-shadow-sm"
          >
            اقوى <br />
            <span className="font-arabic-heavy text-transparent bg-clip-text bg-gradient-to-br from-zinc-900 via-zinc-600 to-zinc-400 italic">
              الكوليكشنز
            </span>
          </motion.h1>
          ...
          >
            {/* Primary Button */}
            <button className="bg-lime-400 cursor-pointer text-black px-8 py-4 rounded-full font-bold uppercase tracking-wider hover:scale-105 hover:shadow-[0_10px_30px_rgba(163,230,53,0.4)] transition-all duration-300 w-full sm:w-auto text-center border border-lime-300">
              عرض كل المنتجات
            </button>
            {/* Secondary Button - Light Mode Version */}
            <button className="bg-white cursor-pointer border border-zinc-200 text-zinc-800 shadow-sm hover:bg-zinc-900 hover:text-white hover:border-zinc-900 px-8 py-4 rounded-full font-bold uppercase tracking-wider transition-all duration-300 w-full sm:w-auto text-center">
              التصفيات
            </button>
          </motion.div>
        </div>

        {/* LEFT SIDE (Single Animated Image) */}
        <div className="hidden lg:flex w-full lg:w-1/2 justify-center items-center relative mt-10 lg:mt-0 h-[40vh] lg:h-auto z-10" dir="ltr">
          <motion.div
            initial={{ opacity: 0, scale: 0.8, rotate: -5, x: -50 }}
            animate={{ opacity: 1, scale: 1, rotate: 0, x: 0 }}
          ...
          >
            ...
            >
              ...
              <div className="relative p-2 md:p-3 bg-white/80 backdrop-blur-xl border border-zinc-200/80 rounded-[2.5rem] shadow-[0_20px_80px_rgba(0,0,0,0.08)] overflow-hidden">
                
                <div className="relative w-56 h-56 md:w-64 md:h-64 lg:w-80 lg:h-80 overflow-hidden rounded-4xl bg-zinc-100">
                  <div className="absolute inset-0 z-10 border border-black/5 rounded-4xl pointer-events-none"></div>
                  ...
                  {/* Softer gradient for light theme image readability */}
                  <div className="absolute inset-0 bg-linear-to-t from-black/60 via-transparent to-transparent z-10"></div>
                </div>

                {/* Light Live Badge */}
                <div className="absolute bottom-6 right-6 flex items-center gap-2 z-20 bg-white/95 px-3 py-1.5 rounded-full border border-zinc-200 shadow-sm backdrop-blur-sm">
                  <div className="w-1.5 h-1.5 rounded-full bg-lime-500 animate-pulse shadow-[0_0_8px_rgba(163,230,53,0.8)]"></div>
                  <span className="font-mono text-[10px] text-zinc-800 font-bold tracking-widest uppercase">LIVE</span>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div 
              animate={{ rotate: 360 }}
              transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
              className="absolute -top-6 -right-6 lg:-right-10 w-24 h-24 lg:w-32 lg:h-32 border border-zinc-300 rounded-full border-dashed pointer-events-none"
            />
            <div className="absolute -bottom-2 -left-4 lg:-bottom-6 lg:-left-10 text-zinc-400 font-mono text-xs lg:text-sm tracking-widest transform -rotate-90 origin-bottom-left pointer-events-none font-semibold">
              // DROP_05_EXCLUSIVE
            </div>
            <div className="absolute top-1/4 -left-4 lg:-left-8 w-1 h-12 bg-lime-400/80 rounded-full pointer-events-none"></div>
          </motion.div>
        </div>
      ...
      {/* --- INFINITE MARQUEE --- */}
      {/* Changed to Dark Background with Lime Text for intense streetwear contrast */}
      <div className="absolute bottom-0 w-full bg-zinc-950 text-lime-400 py-3.5 z-40 flex overflow-hidden border-t border-zinc-800">
        <div className="whitespace-nowrap animate-marquee flex items-center font-bold tracking-widest uppercase text-xs md:text-sm">
           <span>✦ FREE WORLDWIDE SHIPPING OVER $150 ✦ 100% ORGANIC COTTON ✦ NO RESTOCKS ✦ NEXT DROP IN 24:00:00 ✦</span>
           <span className="ml-4">✦ FREE WORLDWIDE SHIPPING OVER $150 ✦ 100% ORGANIC COTTON ✦ NO RESTOCKS ✦ NEXT DROP IN 24:00:00 ✦</span>
           <span className="ml-4">✦ FREE WORLDWIDE SHIPPING OVER $150 ✦ 100% ORGANIC COTTON ✦ NO RESTOCKS ✦ NEXT DROP IN 24:00:00 ✦</span>
